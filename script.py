...
Aktivni_par = global_value.aktivni_par
global_value.seja =0
povzeto={}
zapis={}
dovolim = False
...
def oscar_grind_priprava():
    """ osnovna priprava za izvedbo principa Oscar_gind sistema"""
    #oscar grind
    # cilje je +1 unit
    # vsak trejd je vreden 1
    # pri vsakem negativnem trejdu ki pride en pozitiven mu dodamo 1 unit
    import random
    import time
    ...
    """
    delilec = random.randint(0,5)
    print(delilec)
    print(delilec%2)
    """
    banka = 1010
    procent = 0.02
    vloga = banka * procent #unit
    osnovna_vloga = banka * procent
    profit = banka + (banka*procent)
    #profit = banka + vloga
    print("izracun vloga = ",vloga)
    print("izracun profit = ",profit)
    print("izracun profit2 = ",banka + vloga)
    print("procent: ",procent*100,"%")
    print("\nwhile\n\n")
    while banka > 1 and banka < profit:
        time.sleep(0.5)
        delilec = random.randint(0,5)
        print(delilec)
        print("par " if delilec%2 else "nepar")
        print("true (1)" if delilec%2 else "false (0)")
        print(delilec%2)
        print("banka = ",banka)
        print(">>>>>>>>>>>>>>>>>>")
        if delilec%2:
            print(f"not>> if:: banka: {banka} vloga: {vloga}")
            banka += vloga
            vloga +=osnovna_vloga
            print(f"ven>> if:: banka: {banka} vloga: {vloga}")
        else:
            print(f"not>> else:: banka {banka} vloga {vloga}")
            banka -=vloga
            print(f"ven>> else:: banka {banka} vloga {vloga}")
        print("\n.......\n")
    
    print("\nkonec\n banka po while = ",banka)
    

def oscar_grind():
    print("\n====================\n vhod v oscar grind \n=====================\n\n")
    print(" GV oscar_zakljuceno =  ",global_value.oscar_zakljuceno )
    print(" GV oscar_banka =  ",global_value.oscar_banka )
    print(" GV oscar_profit =  ", global_value.oscar_profit)
    print(" GV oscar_skupaj_vplacano = ",global_value.oscar_skupaj_vplacano)
    print(" GV oscar_skupaj_potrebno_vplacati = ",global_value.oscar_skupaj_potrebno_vplacati)
    #print("\n")
    
    if global_value.oscar_zakljuceno:
        """ dolocim parametre """
        print("\n 1) DOLOCANJE PARAMETROV ")
        global_value.oscar_negativno = False
        global_value.oscar_pozitivno = False
        global_value.oscar_zakljuceno = False
        global_value.oscar_banka = global_value.balance 
        global_value.oscar_procent = 0.01 # 0.02 == 2%
        unit = global_value.oscar_banka * global_value.oscar_procent 
        global_value.oscar_vloga = unit if unit >= 1 else 1 # unit
        global_value.oscar_osnovna_vloga = global_value.oscar_vloga
        global_value.oscar_vplacilo = global_value.oscar_vloga
        global_value.oscar_profit = global_value.oscar_banka + (global_value.oscar_osnovna_vloga *0.91)
        global_value.oscar_skupaj_vplacano += global_value.oscar_vplacilo
        if global_value.oscar_vplacilo == 0:
            global_value.oscar_skupaj_vplacano = 0
            global_value.oscar_skupaj_potrebno_vplacati = 0
        
        
    #elif not global_value.oscar_zakljuceno and global_value.oscar_profit_dosezen:
        #elif global_value.oscar_banka > global_value.balance and global_value.balance > global_value.oscar_profit:
    elif  global_value.balance >= global_value.oscar_profit:
        """ dosezen rezultat z profitom in zakljucimo krog"""
        print(f"\n 2) DOSEZEN PROFIT !! BANKA: {global_value.balance} oscar_banka: {global_value.oscar_banka} oscar_profit: {global_value.oscar_profit}")
        global_value.oscar_zakljuceno = True
        global_value.oscar_skupaj_potrebno_vplacati = 0
        
        """
    elif global_value.oscar_profit > global_value.oscar_banka:
        print(f"\n 3) global_value.oscar_profit > global_value.oscar_banka || profit: {global_value.oscar_profit} banka: {global_value.oscar_banka}")
        global_value.oscar_profit_dosezen = True
        
        """
        global_value.oscar_vplacilo = 0
        global_value.oscar_negativno = False
        global_value.oscar_pozitivno = False
        
    else:
        """ tukaj preverjam uspesnost"""
        if global_value.oscar_pozitivno:
            print("\n 4) OSCAR POZITIVNO")
            global_value.oscar_banka += global_value.oscar_vloga
            global_value.oscar_vloga += global_value.oscar_osnovna_vloga
            print("\n ************ povecanje vloge ***********\n")
            global_value.oscar_vplacilo = global_value.oscar_vloga
        if global_value.oscar_negativno:
            print("\n 5) OSCAR NEGATIVNO")
            global_value.oscar_banka -= global_value.oscar_vloga
            global_value.oscar_vplacilo = global_value.oscar_vloga
    ...
    print("\n\n ------------- dobljeni PODATKI OSCAR GRIND --------------")
    print("\n global_value.oscar_zakljuceno =  ",global_value.oscar_zakljuceno )
    print("\n global_value.BALANCE =  ",global_value.balance )
    print("\n global_value.oscar_banka =  ",global_value.oscar_banka )
    print(" global_value.oscar_profit =  ", global_value.oscar_profit)
    print(" global_value.oscar_procent =  ", global_value.oscar_procent*100,"%")
    print(" global_value.oscar_osnovna_vloga =  ", global_value.oscar_osnovna_vloga)
    print(" global_value.oscar_vloga =  ",global_value.oscar_vloga )
    print(" RETURN global_value.oscar_vplacilo =  ",global_value.oscar_vplacilo )
    print("\nizracun:\nprofit vloga = (profit - banka)/0.91")
    print(f"PREDOGLE: potrebna vloga = ({global_value.oscar_profit} - {global_value.balance}) / 0.91")
    print(f"IZRACUN : potrebna vloga = {(global_value.oscar_profit - global_value.balance)/0.91:.2f}")
    global_value.oscar_skupaj_potrebno_vplacati += (global_value.oscar_profit - global_value.balance)/0.91
    print("\n")
    print("\n>> nova sestevek >> oscar_skupaj_potrebno_vplacati = ", global_value.oscar_skupaj_potrebno_vplacati)
    # v primeru da je vloga prevelika od razlike vloge se mora napraviti nova vloga ki zmanjsa nadaljni rizik
    # (oscar_profit - BANKA ) / 0.91 < oscar_vloga
    # (362,98 - 356,42) / 0,91 < 7,13 
    #  7,208 < 7,13  ==> to je False ( no potrebno korigirati cene vloge)
    # ce je True se uporabi cena izracuna 
    if ((global_value.oscar_profit - global_value.balance) / 0.91) < global_value.oscar_vloga and global_value.oscar_vplacilo:
        global_value.oscar_vloga = (global_value.oscar_profit - global_value.balance) / 0.91
        if global_value.oscar_vloga < 1:
            global_value.oscar_vloga = 1
        print("\n!!! sprememba oscar_vloge !! sedaj znasa = ", global_value.oscar_vloga)
        print("\n>> primerjava >>< oscar_skupaj_potrebno_vplacati = ", global_value.oscar_skupaj_potrebno_vplacati)
        global_value.oscar_vplacilo = global_value.oscar_vloga
        print("\n")
        
    #return  global_value.oscar_vplacilo
    return global_value.oscar_skupaj_potrebno_vplacati
...
def enakost_parov()->str:
    ...
    global_value.aktivni_par = list(global_value.pairs.keys())[0]
    Aktivni_par = global_value.aktivni_par
    if debug:
...
def main():
    funkcija_oznacba_prehod("======= O S C A R  ** G R I N D =======")
    print("\n\n*******************************======= O S C A R  ** G R I N D  =======************************************\n\n")
    povzeto={}
    krog_obrat =0
    ...
    global_value.masaniello_vloge = vloga_masaniello(1,6,1.92,100)
    while True:
        #print("\n************************************************* NOVI KROG ***********************************************\n ",datetime.now())
        ...
        if zadnja_minuta != minuta:
            ...
            print(f"\n\n\n ************************************************************************************************ \
                \n> >IF  v WHILE >> IZDELAN KLJUC: {ura}_minuta_{minuta} || seja: {global_value.seja} || Par: {Aktivni_par} URA: {datetime.now()}\
                \n ***************************************************************************************************\n\n")
            
            print("\nGV.balance:: STANJE BANKE: ",global_value.balance)
            print("\nGV.oscar_banka::  ",global_value.oscar_banka)
            print("\nGV.oscar_profit::  ",global_value.oscar_profit)
            ...
            """ PREVERJANJE IN PRIMERJAVA V PROCENTIH IN NJIHOVA OBNOVA """
            
            
            """ enakost parov """
            preverjen_par = enakost_parov()
            if preverjen_par != Aktivni_par:
                Aktivni_par = preverjen_par
                # par = Aktivni_par
                # menjava =api.change_symbol(Aktivni_par,60)
                # menjava =api.change_symbol(Aktivni_par,30)
                #svece = api.get_candles(Aktivni_par, 60)
                ...
                global_value.obe_bear = False
                global_value.obe_bull = False
                global_value.tri_bear = False
                global_value.tri_bull = False
        ...
        if datetime.now().second > 0 and datetime.now().second < 2:
            print("\n\n ========================================================")
            print(" ~~~~~~~~ ZACETEK URA: ", datetime.now(),"  ~~~~~~~~ " )
            print(" ========================================================\n")
            print("failopenOrder ", global_value.failopenOrder)
            MA10 = moving_average(bars(Aktivni_par), 10)
            if len(MA10) < 4:
                continue
            MA7 = moving_average(bars(Aktivni_par), 7)
            MA3 = moving_average(bars(Aktivni_par), 2)
            C10,T10,D10,P10 = MA10[-4:]
            C7,T7,D7,P7 = MA7[-4:]
            C3,T3,D3,P3 = MA3[-4:]
            
            print("PREHOD **GOR**  MED MA7 IN MA2 = ",P7 < P3 and D7 > D3)
            print("PREHOD **DOL**  MED MA7 IN MA2 = ",P7 > P3 and D7 < D3)
            print("\nPREHOD **GOR**  MED MA10 IN MA 7 = ",P10 < P7 and D10 > D7)
            print("  MA7 JE ZGORAJ = ",P10 < P7)
            print("  MA7 JE SPODAJ = ",P10 > P7)
            print("PREHOD **DOL**  MED MA10 IN MA 7 = ",P10 > P7 and D10 < D7)
            
            print("\npreverjanje ali je trejd odprt ",global_value.trejd_je_odprt)
            print("podatki aktivnega trejda so: ", global_value.oscar_aktivni_trejd)
            trejd_je_odprt = api.get_async_order()
            #>print(trejd_je_odprt)
            print("if >> trejd_je_odprt is not None ",trejd_je_odprt is not None)
            if trejd_je_odprt is not None:
                #print("profit = ",trejd_je_odprt["profit"])
                #print("id = ",trejd_je_odprt["deals"][0]["id"])
                print("id trejda  deals_id = ",deals_id)
                print('if >> deals_id is not trejd_je_odprt["deals"][0]["id"] = ',deals_id is not trejd_je_odprt["deals"][0]["id"])
                if deals_id is not trejd_je_odprt["deals"][0]["id"]:
                    ...
                    if trejd_je_odprt["profit"] == 0 :
                        ...
                        global_value.oscar_pozitivno = False
                        global_value.oscar_negativno = True
                    ...
                    if trejd_je_odprt["profit"] != 0 :
                        ...
                        pozitivni_trejdi += 1
                        #print("negativni trejdi v negativnem PO = ",negativni_trejdi)
                        global_value.oscar_pozitivno = True
                        global_value.oscar_negativno = False
            ...
            if bar1["s"] == "BULL" and bar2["s"] == "BULL":
                print("\ndve sveci sta bull >> ZAPISI V GLOBAL")
                print("cena sredine dveh BULL svec med (2)high in (1)low = ",(bar2["h"] + bar1["l"])/2)
                print("cena sredine dveh BULL svec med (1)high in (2)low = ",(bar2["l"] + bar1["h"])/2)
                print("MA2_P = ",P3)
                print(" ")
                global_value.obe_bear = False
                global_value.obe_bull = True
            if bar1["s"] == "BEAR" and bar2["s"] == "BEAR":
                print("\ndve sveci sta BEAR >> ZAPISI V GLOBAL")
                print("cena sredine dveh BEAR svec med (1)high in (2)low = ",(bar2["l"] + bar1["h"])/2)
                print("cena sredine dveh BEAR svec med (2)high in (1)low = ",(bar2["h"] + bar1["l"])/2)
                print("MA2_P = ",P3)
                print(" ")
                global_value.obe_bear = True
                global_value.obe_bull = False
            if bar1["s"] == "BULL" and bar2["s"] == "BULL" and bar3["s"] == "BULL":
                pass
            if bar1["s"] == "BEAR" and bar2["s"] == "BEAR" and bar3["s"] == "BEAR":
                pass
                
            print("\n")
            print(" GV.obe_bull = ", global_value.obe_bull)
            print(" GV.obe_bear = ", global_value.obe_bear)
            print(" GV.tri_bull = ", global_value.tri_bull)
            print(" GV.tri_bear = ", global_value.tri_bear)
            print("\n")
            
            if global_value.obe_bear: # IZBOR PO SVECAH
            #if D3 < T3: # IZBOR PO MUVING AVERAGE 3 TUKAJ GRESTA DOL<
            #if P3 < T3: # IZBOR PO MUVING AVERAGE 3 TUKAJ GRESTA DOL
                print("\n*************** TUKAJ SEDAJ VSAKA NOVA JE ---- PRODAJA -----\n")
                
                '''
                """ oscar grinf funkcija"""
                vplacilo = oscar_grind()
                if vplacilo <= 0:
                    continue
                '''
                """ MASANIELLO """
                print("stevilo negativnih = ",negativni_trejdi)
                vplacilo = global_value.masaniello_vloge[negativni_trejdi]
                za_smer = "put"
                print(" za smer je = ",za_smer)
                print("vplacano len = ",len(vplacano))
                print("vplacano  = ",vplacano)
                print("preverjanje pred vplacilom ali je trejd odprt ",global_value.trejd_je_odprt)
                vplacano = akcija(vplacilo, Aktivni_par, za_smer, 58)# 1.2<<<
                print("vplacano = ",vplacano)
                print("tip vplacano = ",type(vplacano))
                print("id trejda je = ",vplacano[1])
                global_value.trejd_je_odprt = vplacano[0]
                global_value.oscar_aktivni_trejd = vplacano
                
            if global_value.obe_bull:
            #if D3 > T3:# TUKAJ GRESTA GOR
            #if P3 > T3:# TUKAJ GRESTA GOR
                print("\n*************** TUKAJ SEDAJ VSAKA NOVA JE ---- NAKUP -----\n")
                '''
                """ oscar grinf funkcija"""
                vplacilo = oscar_grind()
                if vplacilo <= 0:
        ...
        if datetime.now().second > 57 and datetime.now().second < 59:
            print(f"\n\n*******************************************************\
                    \n *** ZADNJA SEKUNDA ({datetime.now().second}) *** *** CAS: ({datetime.now()})\
                    \n*********************************************************\n\n")
            MA3 = moving_average(bars(Aktivni_par), 3)
            print(MA3)
...
while krogov < 5:
    ...
    if krogov == 0:
        ...
        print(f"\n\n({krogov})======================= PONOVNO KLICEM main() ========= SPODAJ ================\n\n")
    #mas.koncan_krog=""
    rezultat = main()
    print("\nzakljucen krog ")
