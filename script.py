...
def masaniello():
    ...
    while True:
        ...
        if datetime.now().second > 57 and datetime.now().second < 59:
            ...
            print(f"\n ODLOCITEV = {'KUPI' if T3 < bar0['c'] else 'PRODAJ'} \n")
        ...
        if datetime.now().second > 100 and datetime.now().second < 102:
            ...
            print("\n============= O D L O C A N J E ==========================================\n\n\n")
            sekund: int = int(datetime.now().second)
            razlika_do_konce_minute: int = 60 - sekund
            print("TRENUTNO JE SEKUND: ",sekund ," razlika do 60 = ",razlika_do_konce_minute)
            """
            ...
            ************************>>>> {Aktivni_par} <<<<***********************\n"
            ...
            povzeto["NOV PRISTOP LIST = "] = f"\n\n------- AKTIVNO -------{Aktivni_par}\n\n testiram smer in znesek\
                                                ...
                                                \n se en sestevek = {se_en_sestevek} |> se ena smer = {se_ena_smer} \
            ...
            za_smer =[
                1 if MA3[-4] < bar0['o'] else -1,
                1 if MA3[-3] < bar0['o'] else -1,
                1 if MA3[-1] < bar0['o'] else -1
            ]
            dobim_smer = sum(za_smer)
            smer = se_ena_smer
            print("PREJSNA SMER = ", smer)
            smer = "call" if dobim_smer > 0 else "put" if dobim_smer < 0 else "ne"
            print("nova SMER = ", dobim_smer)
            print("nova SMER = ", smer)
            vloga_ma3 = abs(se_en_sestevek) if se_en_sestevek != 0 else abs(dobim_smer)
            povzeto["preverjanje nove smeri = "] = f"prejsna smer = {se_ena_smer} TRENUTNA nova smer = {smer} "
            
            """ da vidim rezultat hiter """
            hiter =[
            1 if MA3[-2] < bar0['o'] else -1,
            1 if MA2[-1] < bar0['o'] else -1
            ]
            hiter_sum = sum(hiter)
            smer = "call" if hiter_sum > 0 else "put" if hiter_sum < 0 else "ne"
            """ konec hiter rezultat"""
            povzeto["HITER REZULTAT = "] = f"HITER TEST Z MA2 IN MA3 LIST = {hiter}  SUM = {hiter_sum} SMER = {smer}"
            print(povzeto["HITER REZULTAT = "] )
            povprecek_ma2_in_ma3 = (MA3[-1] + MA2[-2]) / 2
            hiter_obrnjen =[
            1 if MA3[-1] < bar0['o'] else -1,
            1 if MA2[-2] < bar0['o'] else -1,
            1 if povprecek_ma2_in_ma3 < bar0['o'] else -1,
            1 if MA2[-2] < MA3[-1] else -1 if MA2[-2] > MA3[-1] else 0
            ]
            hiter_sum_obrnjen = sum(hiter_obrnjen)
            smer_obrnjen = "call" if hiter_sum_obrnjen > 0 else "put" if hiter_sum_obrnjen < 0 else "ne"
            """ konec hiter rezultat"""
            povzeto["HITER REZULTAT obrnjen= "] = f" HITER  TEST Z MA2 IN MA3 LIST = {hiter_obrnjen}  SUM = {hiter_sum_obrnjen} SMER = {smer_obrnjen}"
            print(povzeto["HITER REZULTAT obrnjen= "] )
            smer = smer_obrnjen
            ...
            if smer == "ne":
                ...
                rezultat_ma3 = akcija(vloga_ma3+0.1, Aktivni_par, smer, razlika_do_konce_minute)# 45
            ...
            funkcija_odlocitev(Aktivni_par)
            ...
            ma_7 = ma(bars(par), 7)
            """
            #funkcija_ma(Aktivni_par)
            print("\n************************************************* ")
            print("\nREZULTAT trejda  = rezultat = ",rezultat_ma3)
            print("cas: ",datetime.now())            
            print("\n************************************************* \n")
            if len(rezultat_ma3) == 0:
                print("\n\nNekaj ni vredu z pridobivanjem podatkov rezultat_ma3 je prazen!!\n\nUSTAVLJAM Z quit()\n\n")
                quit()
            #exit(f"\n\n USTAVLJENO DA PREVERIM FUNKCIJO ODLOCITEV {Aktivni_par}\n")
            #quit(f"\nZAKLJUCUJEM PODATKI MA_3 SO >FALSE< ({rezultat_ma3})!!! \n cas = {datetime.now()} ")
            
            aktivacija_akcije = False
            #print("\n------------ AKTIVACJA -----------\n  >> aktivacija_akcije = ",aktivacija_akcije)
